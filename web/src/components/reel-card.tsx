...
export function ReelCard({ reel, onSelect }: { reel: Reel; onSelect?: (reel: Reel) => void }) {
  return (
    <button
      ...
      className="group flex h-full w-full flex-col rounded-xl bg-white p-6 text-left shadow-sm transition-all hover:-translate-y-0.5 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-sphere-purple/50 focus-visible:ring-offset-2"
    ...
    >
      ...
      <div className="mt-auto flex flex-wrap items-center gap-1.5 pt-4">
        {reel.topics.slice(0, 3).map((t) => (
          <span key={t} className="rounded-md bg-sphere-purple/[0.06] px-2 py-0.5 text-xs text-slate-600">
            {t}
          </span>
        ))}
        {reel.permalink && (
