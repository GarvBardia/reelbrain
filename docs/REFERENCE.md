# Mycelium — detailed reference

> The front-door overview is the top-level [README](../README.md). This file is the
> original, deeper reference: the capture API, iOS Shortcut setup, phase-by-phase
> acceptance checks and the full troubleshooting table. Doc names below (e.g.
> `DEPLOYMENT.md`) live in the repo root.
...
| Endpoint | Purpose |
|---|---|
| `GET /api/public/graph` | Graph payload. Defaults to the ~13 **category** nodes; `?expand=<slug>` adds one category's reels, `?expand=all` returns every reel (what the site's `/graph` sphere uses) |
...
**Models:** the code default is `gemini-3.6-flash` (the Render blueprint pins
`gemini-flash-latest`) for extraction+transcription, and
